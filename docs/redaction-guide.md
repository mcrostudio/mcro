...
Duplicate the file (work on the copy).

Regex find/replace using the patterns below (A→D).

Manual sweep for: apiKey, token, secret, password, clientSecret.

Validate JSON (e.g., jq . < file.json or any linter).

Spot-check: secrets gone, structure intact, placeholders present.
...
{ "credentials": { "clientId": "acme-app-123", "clientSecret": "shh-keep-me-safe", "webhookSecret": "whsec_9a8b7c", "aws": { "accessKeyId": "AKIAEXAMPLEKEY123456", "secretAccessKey": "wJalrXUtnFEMI/K7MDENG/bPxRfiCYzEXAMPLEKEY" } } }


AFTER

{ "credentials": { "clientId": "acme-app-123", "clientSecret": "<REDACTED_CLIENT_SECRET>", "webhookSecret": "<REDACTED_WEBHOOK_SECRET>", "aws": { "accessKeyId": "<REDACTED_AWS_ACCESS_KEY_ID>", "secretAccessKey": "<REDACTED_AWS_SECRET>" } } }
